#ifndef _EQSOLVE_PDE_ETD2RKC_C_
#define _EQSOLVE_PDE_ETD2RKC_C_

#include "eqsolve_etd2rkc.h"
#include "complex.h"
#include "math.h"
#include "stdlib.h"
#ifndef pi
...
/*Initiate the ETD2RK complex solver and compute the terms.*/
int eqsolve_etd2rkc_init(
  ...
  if (U_source_->size == solver->numel_U && solver->numel_U != 0) {
    if (L_->size == U_source_->size) {
      ...
      eqsolve_etd2rkc_compute_terms(solver);
    } else {
...
/*Get the current complex ETD2RK solution.*/
...
/*Set the current complex ETD2RK solution.*/
int eqsolve_etd2rkc_set_solution(eqsolve_etd2rkc *solver,
...
/*Compute the complex ETD2RK constant terms.*/
...
int eqsolve_etd2rkc_update(eqsolve_etd2rkc *solver) {
  ...
  for (int i = 0; i < solver->numel_U; i++) {
    solver->U->vector[i] = solver->U_source->vector[i] =
        solver->A->vector[i] +
        solver->T1->vector[i] *
            (solver->N_source->vector[i] - solver->U_N->vector[i]);
  }
...
/*Free the memory allocated for ETD2RK solver.*/
