#ifndef _EQSOLVE_PDE_ETD3RKC_C_
#define _EQSOLVE_PDE_ETD3RKC_C_

#include "eqsolve_etd3rkc.h"
#include "complex.h"
#include "math.h"
#include "stdlib.h"
#ifndef pi
...
/*Allocates memory for complex ETD3RK.*/
eqsolve_etd3rkc *eqsolve_etd3rkc_alloc(int N) {
  eqsolve_etd3rkc *object = malloc(sizeof(eqsolve_etd3rkc));
...
/*Initiate the ETD3RK complex solver and compute the terms.*/
int eqsolve_etd3rkc_init(
    eqsolve_etd3rkc *solver, eqsolve_vector_c *U_source_,
  ...
  if (U_source_->size == solver->numel_U && solver->numel_U != 0) {
    if (L_->size == U_source_->size) {
      ...
      solver->time = 0;
      solver->time_it = 0;
      eqsolve_etd3rkc_compute_terms(solver);
    } else {
...
/*Get the current complex ETD3RK solution.*/
int eqsolve_etd3rkc_get_solution(eqsolve_etd3rkc *solver,
...
/*Set the current complex ETD3RK solution.*/
int eqsolve_etd3rkc_set_solution(eqsolve_etd3rkc *solver,
...
/*Compute the complex ETD3RK constant terms.*/
int eqsolve_etd3rkc_compute_terms(eqsolve_etd3rkc *solver) {
  if (solver->numel_U != 0) {
...
int eqsolve_etd3rkc_update(eqsolve_etd3rkc *solver) {
  if (solver->sf != NULL) {
  ...
  for (int i = 0; i < solver->numel_U; i++) {
    solver->U->vector[i] = solver->U_source->vector[i] =
        ...
        4 * solver->A_N->vector[i] * solver->T2->vector[i] +
...
/*Free the memory allocated for ETD3RK solver.*/
int eqsolve_etd3rkc_free(eqsolve_etd3rkc *solver) {
